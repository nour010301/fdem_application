  ...
  <div class="page-wrapper">
    ...
    <div class="table-wrapper">
      <table v-if="filteredSubdivs.length > 0" class="product-table">
        ...
        <tbody>
          <tr v-for="(item, index) in paginatedSubdivs" :key="item.idSubDivisionNv_1"
              :draggable="canModifySubdivisionOrder(item)"
              @dragstart="onDragStart($event, index)"
              @dragover="onDragOver($event)"
              @drop="onDrop($event, index)"
              :class="{ 'selected-for-move': selectedSubdivisionForMove?.idSubDivisionNv_1 === item.idSubDivisionNv_1, 'non-draggable': !canModifySubdivisionOrder(item) }"
...
const draggedSubdivision = ref<SubdivisionNv1 | null>(null)

// Selection for move buttons
const selectedSubdivisionForMove = ref<SubdivisionNv1 | null>(null)

const search = ref('')
const currentPage = ref(1)
const pageSize = 10

const sortColumn = ref<'nom' | 'designation' | 'idStructure' | 'idSubDivisionNv_1' | 'ordre'>('ordre')
...
function canModifySubdivisionOrder(subdivision: SubdivisionNv1): boolean {
...
function toggleSort(column: 'nom' | 'designation' | 'idStructure' | 'idSubDivisionNv_1' | 'ordre') {
...
async function fetchData() {
  ...
  try {
    ...
    data.value = response.data.sort((a: SubdivisionNv1, b: SubdivisionNv1) => (a.ordre || 0) - (b.ordre || 0))
  } catch (e: any) {
...
async function addSubdivision() {
  try {
    ...
    const res = await axiosInstance.post('subdivision-nv1/', subdivisionToSend)
...
async function reorderSubdivisions(fromIndex: number, toIndex: number) {
  ...
  const updatedSubdivisions: SubdivisionNv1[] = []
  ...
  try {
    ...
    await Promise.all(
      updatedSubdivisions.map(subdivision => 
        axiosInstance.put(`subdivision-nv1/${subdivision.idSubDivisionNv_1}/`, {
          nom: subdivision.nom,
          designation: subdivision.designation,
          idStructure: subdivision.idStructure,
...
// Selection functions
function selectSubdivisionForMove(subdivision: SubdivisionNv1) {
  selectedSubdivisionForMove.value = subdivision
}

const canMoveUp = computed(() => {
  if (!selectedSubdivisionForMove.value) return false
  const currentIndex = filteredSubdivs.value.findIndex(s => s.idSubDivisionNv_1 === selectedSubdivisionForMove.value!.idSubDivisionNv_1)
...
function exportCSV() {
  const headers = ['ID', 'Nom', 'Désignation', 'ID Structure', 'Ordre', 'Plan Vérif']
